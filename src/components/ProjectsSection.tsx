...
import { SITE_ACTION_EVENT, type SiteAction } from "@/lib/site-actions";
...
export default function ProjectsSection() {
  ...
  // Ações disparadas pelo chat de IA ([[projeto:<id>]]) abrem o modal daqui
  useEffect(() => {
    const onAction = (e: Event) => {
      const action = (e as CustomEvent<SiteAction>).detail;
      if (action?.type !== "project") return;
      const project = projects.find(p => p.id === action.id);
      if (!project) return;
      document.getElementById("projetos")?.scrollIntoView({ behavior: "smooth", block: "start" });
      setSelected(project);
    };
    window.addEventListener(SITE_ACTION_EVENT, onAction);
    return () => window.removeEventListener(SITE_ACTION_EVENT, onAction);
  }, []);
